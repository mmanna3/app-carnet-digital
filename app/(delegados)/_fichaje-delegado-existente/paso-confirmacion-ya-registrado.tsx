...
export default function PasoConfirmacionYaRegistrado() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const result = await obtenerNombreUsuarioVerde()
  ...
  return (
    <View testID="paso-confirmacion-ya-registrado" className="flex-1 bg-surface">
      <Cabecera titulo="Fichaje de delegado" onBack={() => irAlPasoAnterior()} />
      <ProgresoDelegado />
      ...
      <ScrollView className="flex-1 px-6 pt-6" contentContainerStyle={{ paddingBottom: 24 }}>
        <View className="mb-6">
          ...
          <Text className="text-zinc-400 text-sm">
            Fichando como delegado del club{' '}
            <Text className="font-bold text-zinc-100">{nombreClub}</Text>
          </Text>
        </View>
        ...
        <View className="glass rounded-2xl border border-border-glass p-5 mb-6">
          {cargandoUsuario ? (
            <View className="items-center py-4">
              ...
              <Text className="text-zinc-400 text-sm mt-2">Obteniendo nombre de usuario...</Text>
            </View>
          ) : errorNombreUsuario ? (
            <View>
            ...
            <Text className="text-zinc-400 text-sm leading-relaxed">
              ...
              nombre de usuario: <Text className="font-bold text-zinc-100">{nombreUsuario}</Text>.
              La contraseña es tu DNI.
            </Text>
          ) : (
            <Text className="text-zinc-400 text-sm leading-relaxed">
              Una vez que la administración de la liga apruebe, vas a poder iniciar sesión como
              delegado del club <Text className="font-bold text-zinc-100">{nombreClub}</Text>. La
