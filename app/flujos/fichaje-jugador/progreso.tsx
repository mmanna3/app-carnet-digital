...
export default function Progreso() {
  ...
  return (
    <View
      className={`${esDelegado && 'pt-4'} px-6 pt-1 pb-6 bg-surface border-b border-border-glass flex-row items-center justify-center`}
    >
        ...
        <View key={paso} className="flex-row items-center">
          <View
            className={`w-9 h-9 rounded-full border-2 items-center justify-center ${
              paso <= pasoActual ? 'bg-liga-600 border-liga-600' : 'bg-surface-elevated border-border-glass'
            }`}
          >
          ...
          {index < totalPasos - 1 && (
            <View className={`h-0.5 w-6 ${paso < pasoActual ? 'bg-liga-600' : 'bg-border-glass'}`} />
          )}
        </View>
      ))}
    </View>
