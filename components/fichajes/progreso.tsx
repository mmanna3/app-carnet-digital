...
export default function Progreso({ totalPasos, pasoActual }: Props) {
  return (
    <View className="px-6 py-3 bg-white border-b border-gray-100 flex-row items-center justify-center">
      {Array.from({ length: totalPasos }, (_, i) => i + 1).map((paso, index) => (
        <View key={paso} className="flex-row items-center">
          ...
          >
            <Text className={`text-xs ${paso <= pasoActual ? 'text-white' : 'text-gray-400'}`}>
              {paso}
            </Text>
          </View>
          {index < totalPasos - 1 && (
